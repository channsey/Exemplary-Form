...
create table One (
  col_A varchar(10),
  col_B varchar(2) unique,
  col_D integer unique,
  col_E integer unique,
  col_I integer,
  col_J integer,
  col_K integer,
  col_L varchar(3),
...
create table Two (
  ...
  --primary key (col_B),
  foreign key (col_B) references One (col_B)
    on delete cascade
);
create table Three (
  col_E integer unique,
  col_F integer	unique,
  --primary key (col_E),
  foreign key (col_E) references One (col_E)
    on delete cascade
);
create table Four (
  col_F integer unique,
  ...
  col_H varchar(3),
  --primary key (col_F),
  foreign key (col_F) references Three (col_F)
    on delete cascade
);
create table Five (
  col_V integer,
  col_X varchar(1) unique,
  --primary key (col_X),
  foreign key (col_X) references One (col_X)
    on delete cascade
);
create table Six (
  col_R integer,
  ...
  col_U integer,
  unique (col_R, col_S),
  --primary key (col_R, col_S),
  foreign key (col_R, col_S) references One (col_R, col_S)
    on delete cascade
);
/*
...
-- NOT FINISHED YET
/*
Old version had the foreign key relation backwards
& duplicated a lot of data that's supposed to be unique
Ex: Two's insert statements simplify to ->
...
drop table if exists Three cascade;
drop table if exists Four cascade;
drop table if exists Two cascade;
drop table if exists Five cascade;
drop table if exists Six cascade;

/*
drop table One cascade;
drop table Two cascade;
drop table Three cascade;
drop table Four cascade;
drop table Five cascade;
drop table Six cascade;
*/
create table One (
  col_A varchar(10),
  col_B varchar(2) unique,
  ...
  unique (col_R, col_S),
  primary key (col_D),
  foreign key (col_B) references Two (col_B),
    on delete cascade
  foreign key (col_E) references Three (col_E),
    on delete cascade
  foreign key (col_X) references Five (col_X),
    on delete cascade
  foreign key (col_R, col_S) references Six (col_R, col_S)
    on delete cascade
);
create table Two (
  col_B varchar(2) unique,
...
create table Three (
  ...
  col_F integer	unique,
  primary key (col_E),
  foreign key (col_F) references Four (col_F)
    on delete cascade
